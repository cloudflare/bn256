// Copyright 2012 The Go Authors. All rights reserved. Use of this source code is governed by a BSD-style license that
...
func (e *GT) Finalize() *GT {
	ret := finalExponentiation(e.p, new(bnPool))
	e.p.Set(ret)
	return e
}
...
// bnPool implements a tiny cache of *big.Int objects that's used to reduce the number of allocations made during
// processing.
type bnPool struct {
	bns   []*big.Int
	count int
...
func (pool *bnPool) PutAll(bns []*big.Int) {
	if pool == nil {
		return
	}
	pool.bns = append(pool.bns, bns...)
	pool.count -= len(bns)
}
